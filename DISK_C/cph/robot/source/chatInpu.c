...
void DeleteTab(Coordinate * current_en_position,CH* ch,EN* en,char *temp ,int * num_tab,int *ch_qhwh, unsigned int *box_save)//3
{
    int en_qhwh,i;
...
void ShowTxt(int *qhwh, Area show_area,Coordinate* current_show_position,int show_size)//6
{
    ...
    {
        if(qhwh[i]!=0)	//打印整句话
        ShowPerCharacter(qhwh[i],show_area, current_show_position,36410 , show_size,"hzk");
        else
        {
            ShowWhite( current_show_position, show_size);
...
int ShowChTab(Area show_area,Coordinate current_show_position/*现在正在码的字的左上角坐标*/, int show_size/*有48，36，28*/,int *qhwh,unsigned int *box_save)//7
{
    union
    {
        int key;
    ...
    while(1)//进入按键循环
    {
		...
		**********/		
        ...
        {
            DeleteTab(& current_en_position, ch, en, temp ,& num_tab,& ch_qhwh, box_save);
        }
